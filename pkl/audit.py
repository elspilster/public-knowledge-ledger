...
def _canonical_event_data(event_id: str, event_type: str, object_id: str, timestamp: str, payload: dict[str, Any], previous_hash: str) -> bytes:
    data = {"event_id": event_id, "event_type": event_type, "object_id": object_id, "timestamp": timestamp, "payload": payload, "previous_hash": previous_hash}
...
def calculate_hash(event_id: str, event_type: str, object_id: str, timestamp: str, payload: dict[str, Any], previous_hash: str) -> str:
    return hashlib.sha256(_canonical_event_data(event_id, event_type, object_id, timestamp, payload, previous_hash)).hexdigest()
...
class AuditChain:
    ...
    def append(self, event_id: str, event_type: str, object_id: str, timestamp: str, payload: dict[str, Any], *, signer_key_id: str | None = None, signature: str | None = None) -> AuditEvent:
    ...
    def verify(self) -> bool:
        ...
        for event in self.events:
            expected = calculate_hash(event.event_id, event.event_type, event.object_id, event.timestamp, event.payload, previous_hash)
    ...
    def verify_signatures(self, public_keys: dict[str, bytes]) -> bool:
        """Verify every signed event; unsigned events are accepted for legacy compatibility."""
        ...
        for event in self.events:
            ...
            if public_key is None or not verify_event_signature(public_key, event.signature, event.event_id, event.event_type, event.object_id, event.timestamp, event.payload, event.previous_hash):
                return False
        return True
