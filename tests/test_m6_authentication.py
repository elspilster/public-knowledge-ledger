...
def signed_chain(signer):
    chain = AuditChain()
    event = chain.append("EVT-1", "claim.created", "PKL-1", TIMESTAMP, PAYLOAD, signer_key_id=signer.key_id)
    ...
    chain.events[0] = replace(event, signature=signature)
    return chain
...
def test_signature_substitution_is_rejected():
    ...
    chain = signed_chain(signer)
    attacker_signature = sign_event(attacker, chain.events[0].event_id, chain.events[0].event_type, chain.events[0].object_id, chain.events[0].timestamp, chain.events[0].payload, chain.events[0].previous_hash)
